"""IBTrACS-to-ATCF/ASWIP text conversion for the training-data generator.

Split out of ``adforce/generate_training_data.py``. These functions turn a
single-storm IBTrACS ``xarray.Dataset`` into the two text inputs each ADCIRC
run needs:

- ``convert_ibtracs_storm_to_aswip_input`` writes ``pre_aswip_fort.22``, the
  fixed-width FORMAT(22) file that ASWIP converts to an NWS=20 (GAHM)
  ``fort.22``.
- ``convert_ibtracs_storm_to_atcf`` writes ``atcf.txt``, the comma-separated
  ATCF best-track file that adcircpy's ``BestTrackForcing`` reads to build
  the ``fort.15`` metadata.

``_decode_char_array`` and ``clean_radii`` are the pure helpers they share.
Only numpy/pandas/xarray are needed here (no adcircpy), so this module is
cheap to import and its doctests run under ``pytest --doctest-modules``.
"""
...
def clean_radii(radii_nm: list) -> list:
    ...
    Note: mixed (some-zero) input lists are modified in place and the same
    list object may be returned.
...
def convert_ibtracs_storm_to_aswip_input(ds: xr.Dataset, output_atcf_path: str):
    ...
    Side effects: writes the fixed-width file to ``output_atcf_path`` and
    prints progress; an ``IOError`` on write is caught and only printed.
...
def convert_ibtracs_storm_to_atcf(ds: xr.Dataset, output_atcf_path: str):
    ...
    Side effects: writes the comma-separated ATCF file to ``output_atcf_path``
    (later read back by adcircpy's ``BestTrackForcing``) and prints progress.
    Unlike the ASWIP twin, missing variables (e.g. 'usa_poci'/'usa_roci')
    raise, and no ``clean_radii`` correction is applied.
