...
def make_trend_fig(out_dir: str | None = None) -> None:
    ...
    bias, width = _panel(
        dt,
        "trend",
        r"Upper-bound trend, $dz^*/dt$ [m yr$^{-1}$]",
    ...
    for rp in dt.rp.values:
        ...
        print(
            f"    trend=0.01  RV{int(rp)}: I-ns bias "
...
def make_sigma_fig(out_dir: str | None = None) -> None:
    ...
    _panel(
        dss,
        "sigma",
        r"Bound level uncertainty $\sigma_{z^*}$ [m]",
    ...
    for rp in dss.rp.values:
        ...
        if below.all():
            ...
            cx = float(
                np.interp(0, [rI[i - 1] - rII[i - 1], rI[i] - rII[i]], [s[i - 1], s[i]])
            )
