...
def _synth_year(year: int, storm_peak: float, seed: int) -> pd.Series:
    ...
    tide = (0.25 * np.cos(2 * np.pi * t / M2_HR + 0.3)
            + 0.15 * np.cos(2 * np.pi * t / K1_HR + 1.1))
...
def test_pipeline_recovers_storm_annual_maxima(synth_pipeline):
    ...
    for _, row in df.iterrows():
        ...
        assert abs((row.t_max - pd.Timestamp(f"{int(row.year)}-09-15"))
...
def gev_sample() -> np.ndarray:
    ...
    data = genextreme.rvs(c=-GAMMA_TRUE, loc=ALPHA_TRUE, scale=BETA_TRUE,
...
def test_fit_cases_recover_truth_and_bound(gev_sample):
    ...
    assert float(return_level(a1, b1, g1, 100.0)) == pytest.approx(
        rv100_true, abs=0.25)
    assert float(return_level(a2, b2, g2, 100.0)) == pytest.approx(
        rv100_true, abs=0.4)
...
def test_bootstrap_bands_finite_and_bounded(gev_sample):
    ...
    b1 = bootstrap_bands(fits["I"], "I", len(gev_sample), z_star=Z_STAR,
                         n_boot=40, rps=rps, seed=1)
    b2 = bootstrap_bands(fits["II"], "II", len(gev_sample), n_boot=40,
                         rps=rps, seed=1)
    for b in (b1, b2):
...
# --------------------------------------------------------------------------- #
@pytest.mark.skipif(os.environ.get("WORSTSURGE_NETWORK_TESTS") != "1",
