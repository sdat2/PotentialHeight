...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("mu,sig,xi", [(2.0, 1.0, -0.2), (0.0, 0.5, -0.1), (5.0, 2.0, -0.35)])
...
def test_return_level_matches_scipy_genextreme(mu, sig, xi, T):
    ...
    assert ours == pytest.approx(scipy_rl, abs=1e-10), (
...
def test_return_level_monotone_and_bounded_by_endpoint():
    ...
    assert ZSTAR - zs[-1] < 0.1, f"z_p at p=1e-9 should be within 0.1 of z*={ZSTAR}, got {zs[-1]}"
...
def test_rl_from_endpoint_consistent_with_return_level():
    ...
    for psi, sig, xi, p in [(7.0, 1.0, -0.2, 0.002), (3.0, 0.4, -0.1, 0.01), (10.0, 2.0, -0.3, 0.05)]:
        a = _rl_from_endpoint(psi, sig, xi, p)
        b = return_level(psi + sig / xi, sig, xi, p)
        assert a == pytest.approx(b, abs=1e-12), f"endpoint parameterisation mismatch: {a} vs {b}"
...
def test_fisher_information_symmetric_positive_definite(I3):
    ...
    np.testing.assert_allclose(I3, I3.T, atol=1e-8, err_msg="Fisher information not symmetric")
...
def test_asymptotic_se_zstar_matches_plot_reference(I3):
    ...
    assert se50 == pytest.approx(1.8, abs=0.15), f"SE(z*, n=50) = {se50}, expected ~1.8 m"
...
def test_cornish_fisher_RII_positive_and_shrinks_like_sqrt_n(I3):
    ...
    assert r100 / r400 == pytest.approx(2.0, rel=0.1), (
...
def test_gumbel_plateau_matches_cached_reference(plateau):
    ...
    for T in (100, 500):
        assert b_inf[T] == pytest.approx(B_INF_REF[T], abs=0.06), (
            f"Gumbel plateau b_inf[{T}] = {b_inf[T]} drifted from reference {B_INF_REF[T]}"
        )
    assert b_inf[500] > b_inf[100] > 0, "plateau should be positive and grow with return period"
...
def test_saturating_bias_monotone_and_below_plateau(sat, plateau):
    ...
    for T in (100, 500):
        ...
        assert np.all(np.diff(b) > 0), f"bias not increasing in the margin u for rp={T}: {b}"
        ...
        assert np.all(b < b_inf[T] * 1.02), f"bias exceeds the plateau for rp={T}: {b} vs {b_inf[T]}"
        assert b[-1] > 0.6 * b_inf[T], (
...
def test_fisher_slope_matches_cached_reference(sat):
    _, cp = sat
    for T in (100, 500):
        assert cp[T] == pytest.approx(C_P_REF[T], rel=0.1), (
...
def test_simulate_crossover_outputs_sane(cross, I3):
    ...
    assert np.isfinite(rII) and rII > 0, f"unbounded range R_II not positive finite: {rII}"
...
def test_closed_form_crossover_agrees_with_simulation(cross):
    ...
    assert np.isfinite(cx_sim) and cx_sim > 0, f"simulated crossover not finite: {cx_sim}"
    assert np.isfinite(cx_cf) and cx_cf > 0, f"closed-form crossover not finite: {cx_cf}"
    ratio = cx_cf / cx_sim
    assert 0.7 < ratio < 3.5, (
...
def test_simulate_crossover_deterministic():
    ...
    assert a[0] == b[0] and a[1] == b[1], "R_II / SE not deterministic under the internal seed"
    assert (np.isnan(a[2]) and np.isnan(b[2])) or a[2] == b[2], "crossover not deterministic"
    np.testing.assert_array_equal(a[4], b[4], err_msg="bounded ranges not deterministic")
...
def test_bounded_fit_clips_bound_to_data_max():
    ...
    assert np.isfinite(z_bnd), "bounded fit with an infeasible bound should still be finite"
    assert z_bnd < d.max() + 1e-5, f"bounded return level {z_bnd} above the clipped endpoint"
    z_unb = _fit_unb(d, p)
