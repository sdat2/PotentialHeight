"""Run the ERA5 potential-size calculation on an ARCHER2 dask cluster.

Everything lives behind a ``__main__`` guard so that importing this module
(e.g. by sphinx autodoc or ``pytest --doctest-modules``) does not try to
...
def main() -> None:
    """Set up the SLURM dask cluster and run the potential-size calculation."""
    ...
    try:
        ...
        print(
            f"Cluster is ready. Found {len(client.scheduler_info()['workers'])} workers."
        )
