...
def load_cmip6_data(
    exp="historical", model="CESM2", member="r4i1p1f1"
) -> Optional[xr.Dataset]:
...
def calc_bias(
    ...
    vars_to_compare = (
        set(era5_ds.data_vars)
        .intersection(set(era5_ds.data_vars))
        .intersection(["sst", "vmax", "pmin", "otl"])
    )
...
def read_ensemble_for_point_var(
    ...
    """
    print(
        f"Reading data for {model}, {member}, {exp}, {var} at lat={lat}, lon={lon}..."
    )
    if isinstance(model, str):
        if isinstance(member, str):
            if isinstance(exp, list):
                if len(exp) == 1:
                    ...
                    if historical_ds is not None and ssp_ds is not None:
                        print(
                            f"Combining historical and {exp[non_hist_index]} datasets."
                        )
                        return xr.concat(
                            [historical_ds, ssp_ds],
                            dim="year",
                            coords="minimal",
                            compat="override",
                        )
                    ...
                    print(
                        f"Multiple experiments {exp} found, please specify one, or two if one of them is historical."
                    )
                    return None
            ...
            if cmip6_ds is not None:
                ...
                cmip6_ds = select_seasonal_hemispheric_data(
                    cmip6_ds, lon="lon", lat="lat"
                )
            ...
            if output is not None:
                ...
                if isinstance(member, list):
                    output = output.assign_coords(
                        member=[f"{m}.{mem}" for mem in output.member.values]
                    )
                else:
...
def example_new_orleans_plot(start_year=1980, end_year=2024) -> None:
    ...
    era5_ds = (
        get_all_regridded_data(start_year=start_year, end_year=end_year)
        .sel(time=slice(f"{start_year}-01-01", f"{end_year}-12-31"))["vmax"]
        .sel(lat=29.95 - 1, lon=-90.07, method="nearest")
    )
    ...
    axs[0].axhline(
        era5_ds.mean(dim="year").values, color="blue", linestyle="--", label="ERA5 Mean"
    )
    print("CMIP6 data:", cmip6_ds)
    cmip6_ds.plot.line(
        ax=axs[0], x="year", label="CMIP6", color="orange", linewidth=0.5
    )  # x="year",
    axs[0].axhline(
        cmip6_ds.mean(dim="year").mean(dim="member").values,
        color="orange",
        linestyle="--",
        label="CMIP6 Mean",
    )
    ...
    axs[1].axhline(
        bias_ds.mean(dim="year").mean(dim="member").values,
        color="red",
        linestyle="--",
        label="Bias mean",
    )
