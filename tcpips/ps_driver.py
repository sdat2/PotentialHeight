...
f1".
    ...
    ds = xr.merge([oc_ds, at_ds_aligned, pi_ds_aligned]).chunk({"time": time_chunk, "plev": -1})
    ds = convert(ds)
    ...
    vars = [var for var in ds.variables if var not in ["r0_cat1", "rmax_cat1", "vmax_cat1", "r0_pi", "rmax_pi", "vmax_pi"]]
