...
STORMS=[("KATRINA","NA"),("IDA","NA"),("HELENE","NA"),("IAN","NA"),("HARVEY","NA"),("FIONA","NA"),
        ("SAOLA","WP"),("MANGKHUT","WP"),("HATO","WP"),("VICENTE","WP"),("YORK","WP"),("ELLEN","WP"),
        ("BEBINCA","WP"),("JEBI","WP"),("MERANTI","WP"),("FREDDY","SI")]
# --- identify storm indices by name+basin ---
base=xr.open_dataset(f"{DST}/IBTrACS.since1980.v04r01.nc")
def decode(arr):
    v=arr.values
    if v.ndim==2: return np.array(["".join(c.decode() if isinstance(c,bytes) else str(c) for c in row).strip() for row in v])
    return np.array([x.decode().strip() if isinstance(x,bytes) else str(x).strip() for x in v])
names=decode(base["name"]); basins=decode(base["basin"])
S=base.sizes["storm"]; D=base.sizes["date_time"]
want=set()
for nm,bs in STORMS:
    hit=np.where((names==nm)&(basins==bs))[0]
    want.update(hit.tolist())
want=sorted(want); print(f"matched {len(want)} storms for {len(STORMS)} (name,basin) targets",flush=True)
...
for s in want: mask[s*D:(s+1)*D]=True
def one(vmax,i):
    import w22.ps as p
...
for meas,vmax in vmap.items():
    idx=np.where(mask&np.isfinite(vmax)&(vmax>0.01)&np.isfinite(sst)&np.isfinite(rh)&(rh>=0)&(rh<=1))[0]
    t=time.time()
    res=Parallel(n_jobs=3,backend="loky")(delayed(one)(vmax,i) for i in idx)
    # patch into copy
    nc=Dataset(f"{DST}/{fname[meas]}","a");rm=nc.variables["rmax"];orig=np.asarray(rm[:],dtype="float64");sh=orig.shape
    flat=orig.ravel();flat[idx]=res;rm[:]=flat.reshape(sh);nc.close()
    print(f"[{meas}] re-solved+patched {len(idx)} track pts in {(time.time()-t)/60:.1f} min",flush=True)
# --- plot each storm ---
import tcpips.ibtracs as ib
ok=0
for nm,bs in STORMS:
    try:
        ib.plot_tc_example(name=nm.encode(),basin=bs.encode(),subbasin=b"GM" if bs=="NA" else b"WPAC",bbox=None)
        ok+=1
    except Exception as e:
        print(f"  {nm} FAIL: {type(e).__name__}: {str(e)[:120]}")
print(f"\nplotted {ok}/{len(STORMS)} track figs")
import glob
print("tracks dir:", len(glob.glob(os.path.join(ib.FIGURE_PATH,"tracks","*.pdf"))), "pdfs")
print("DONE")
