...
SRC="/Volumes/s/tcpips/data/ibtracs"; DST="/Volumes/s/tcpips/fixed_ibtracs"; SC=os.path.dirname(__file__)
...
for meas,fn in fname.items():
    exact=np.load(f"{SC}/exact_{meas}.npz"); W=exact["W"].astype(bool); rf_cat1=exact["rmax_fixed"]
