...
cps=xr.open_dataset(f"{IB}/IBTrACS.since1980.v04r01.cps.nc")[["vmax","msl","sst","t0","rh","ck_cd","cd"]].load()
r=lambda n:np.asarray(cps[n].values,dtype="float64").ravel()
vmax,msl,sst,t0,rh,ck,cd=[r(n) for n in ("vmax","msl","sst","t0","rh","ck_cd","cd")]
lat=np.asarray(cps['lat'].values,dtype='float64').ravel()
if np.nanmax(rh)>1.5: rh=rh/100
idx=np.where(np.isfinite(vmax)&(vmax>0.01)&np.isfinite(sst)&np.isfinite(rh))[0][:500]
def one(i):
    import w22.ps as p
...
print(f"n_jobs=4: {len(out)} solves in {time.time()-t:.0f}s ({np.isfinite(out).sum()} finite)")
