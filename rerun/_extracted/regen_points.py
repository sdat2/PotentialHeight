...
MODELS={"CESM2":["r4i1p1f1","r10i1p1f1","r11i1p1f1"],
    ...
    for model,members in MODELS.items():
        for mem in members:
            for exp in ["historical","ssp585"]:
                out=os.path.join(DATA_PATH,f"{place}_august_{exp}_{model}_{mem}_isothermal_pi4new.nc")
                if os.path.exists(out): skip+=1; continue
                try:
                    t=time.time()
                    point_timeseries(member=mem,model=model,recalculate_pi=True,exp=exp,place=place,pressure_assumption="isothermal",pi_version=4)
                    done+=1; print(f"OK {place}/{model}/{mem}/{exp} ({(time.time()-t):.0f}s) [done={done} skip={skip} fail={fail}]",flush=True)
                except Exception as e:
                    fail+=1; print(f"FAIL {place}/{model}/{mem}/{exp}: {type(e).__name__}: {str(e)[:150]}",flush=True)
