...
def solve(vmax, rh, sst, msl, t0, lat, buggy):
    ...
    try:
        ds = xr.Dataset(dict(sst=sst, supergradient_factor=1.2, t0=t0, w_cool=0.002,
                             vmax=vmax, msl=msl, rh=rh, cd=0.0015), coords=dict(lat=lat))
...
def inp(ds, yr):
    ...
    return dict(vmax=g('vmax_3','vmax'), rh=g('rh'), sst=g('sst'), msl=g('msl'), t0=g('t0'), lat=lat)

print("="*78)
print("PART 1 — isolate the humidity fix (identical inputs, buggy vs fixed)")
print("="*78)
for label, path in [("PAPER point (lat 29.25)", f"{SSD}/new_orleans_august_ssp585_r4i1p1f1_isothermal_pi4new.nc"),
                    ("CURRENT point (lat 28.75)", f"{DATA}/new_orleans_august_ssp585_CESM2_r4i1p1f1_isothermal_pi4new.nc")]:
    ds = xr.open_dataset(path)
    print(f"\n{label}:")
    for yr in (2015, 2100):
        iv = inp(ds, yr)
        b = solve(**{k:iv[k] for k in ('vmax','rh','sst','msl','t0','lat')}, buggy=True)
        f = solve(**{k:iv[k] for k in ('vmax','rh','sst','msl','t0','lat')}, buggy=False)
        print(f"  {yr} (rh={iv['rh']:.3f}): r0 {b[0]:.1f}->{f[0]:.1f} km ({100*(f[0]/b[0]-1):+.1f}%), "
              f"rmax {b[1]:.2f}->{f[1]:.2f} km ({100*(f[1]/b[1]-1):+.1f}%)")

print("\n" + "="*78)
print("PART 2 — fresh regen (fixed, current pt) vs archived (buggy, paper pt), by city")
print("="*78)
cities = [("new_orleans", f"{SSD}/new_orleans_august_ssp585_r4i1p1f1_isothermal_pi4new.nc"),
...
for city, oldpath in cities:
    ...
    if not os.path.exists(newpath):
        print(f"\n{city}: fresh file not present yet"); continue
    hasold = os.path.exists(oldpath)
    new = xr.open_dataset(newpath); old = xr.open_dataset(oldpath) if hasold else None
    print(f"\n{city}: new pt lat={float(new.lat.values)}" + (f" | archived pt lat={float(old.lat.values)}" if hasold else " | (no archived file)"))
    for yr in (2015, 2100):
        ni = inp(new, yr)
        ...
        if hasold:
            oi = inp(old, yr)
            oldr0 = float(old.isel(time=[t.year==yr for t in old.indexes['time']]).r0.values.ravel()[0])/1000
