...
vmax=r(cps,"vmax");sst=r(cps,"sst");rh=r(cps,"rh");usa_rmw=r(nrm,"usa_rmw");usa_wind=r(nrm,"usa_wind");Vp=r(pip,"vmax")
if np.nanmax(rh)>1.5: rh=rh/100
# comparison set: has observation (usa_rmw) + valid inputs
base=np.isfinite(vmax)&(vmax>0.01)&np.isfinite(sst)&np.isfinite(rh)&(rh>=0)&(rh<=1)&np.isfinite(usa_rmw)&(usa_rmw>0)
...
# time 800 solves
idx=np.where(base)[0]; samp=idx[:800]
msl=r(cps,"msl");t0=r(cps,"t0");lat=np.asarray(cps['lat'].values,dtype='float64').ravel();ck=r(cps,"ck_cd");cd=r(cps,"cd")
psmod.calculate_ps_ufunc(vmax[samp[0]],msl[samp[0]],sst[samp[0]],t0[samp[0]],lat[samp[0]],rh[samp[0]],0.9,0.0015,0.002,1.2,"isothermal") # warmup
t=time.time()
for i in samp:
    psmod.calculate_ps_ufunc(vmax[i],msl[i],sst[i],t0[i],lat[i],rh[i],ck[i] if np.isfinite(ck[i]) else 0.9,cd[i] if np.isfinite(cd[i]) else 0.0015,0.002,1.2,"isothermal")
dt=(time.time()-t)/len(samp)
print(f"\nsteady-state: {dt*1000:.1f} ms/solve (single-thread)")
n=int(base.sum())
print(f"full exact r2 re-solve ({n} pts): serial ~{n*dt/60:.0f} min; n_jobs=4 ~{n*dt/4/60:.0f} min")
print(f"all 3 measures (~3x): serial ~{3*n*dt/60:.0f} min; n_jobs=4 ~{3*n*dt/4/60:.0f} min")
