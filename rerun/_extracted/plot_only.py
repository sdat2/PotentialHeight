...
def step(name, fn):
    ...
    except Exception as e: print(f"FAIL {name}: {type(e).__name__}: {e}",flush=True); traceback.print_exc(limit=4)
step("multipanel new_orleans", lambda: wp.multipanel(place="new_orleans"))
step("multipanel hong_kong", lambda: wp.multipanel(place="hong_kong", models={"HADGEM3-GC31-MM","MIROC6","ERA5"}))
step("plot_era5_timeseries new_orleans", lambda: wp.plot_era5_timeseries(place="new_orleans"))
step("temporal_relationship_data new_orleans", lambda: st.temporal_relationship_data(place="new_orleans"))
import glob
print("FIGS:", [os.path.basename(f) for f in glob.glob(os.path.join(wp.FIGURE_PATH,"*.pdf")) if os.path.getmtime(f)>__import__('time').time()-200])
print("DONE")
