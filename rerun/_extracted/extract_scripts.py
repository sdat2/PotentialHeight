...
for base, (body, _) in sorted(scripts.items()):
    with open(os.path.join(OUT, base), "w") as g:
        g.write(body + "\n")
for base, ct in writes.items():
    if base not in scripts:
        with open(os.path.join(OUT, "write_" + base), "w") as g:
            g.write(ct)

print(
    f"extracted {len(scripts)} heredoc scripts + {len([b for b in writes if b not in scripts])} Write-only .py"
)
for b in sorted(scripts):
    print("  ", b, f"({len(scripts[b][0])} bytes)")
