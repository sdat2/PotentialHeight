...
os.environ["HDF5_USE_FILE_LOCKING"]="FALSE"; os.environ["LOKY_MAX_CPU_COUNT"]="3"
...
print(f"get_all_data(1980,1989): {'None' if ds is None else 'loaded (lazy)'} in {time.time()-t0:.0f}s")
...
print("PS-input vars present:", [v for v in ("vmax_3","vmax_1","sst","msl","rh","t0") if v in ds])
# count valid ocean cells across the decade (small: just vmax_3, lazy->compute one var)
vmax=np.asarray(ds["vmax_3"].values); nvalid=int(np.sum(np.isfinite(vmax)&(vmax>0.01)))
...
tt=time.time(); out=parallelized_ps13_dask(sub); out=out.compute() if hasattr(out,'compute') else out
dt=time.time()-tt
sv=np.asarray(sub["vmax_3"].values); nsub=int(np.sum(np.isfinite(sv)&(sv>0.01)))
rate=dt/max(nsub,1)
print(f"subset {dict(sub.sizes)}: {nsub} valid cells in {dt:.1f}s -> {rate*1000:.0f} ms/cell")
