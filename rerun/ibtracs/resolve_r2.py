...
os.environ["HDF5_USE_FILE_LOCKING"] = (
    "FALSE"  # exFAT/macOS: netCDF4/h5py file locking must be off
)
os.environ.setdefault(
    "LOKY_MAX_CPU_COUNT", "4"
)  # cap joblib workers on macOS (memory guard)
...
print(
    "[GATE] r2 (normalized_size_cps), per-storm-max survival-at-1, Cat-1 wind filter:"
)
...
fv, fm, fs, ft, fr, frx, fl, fck, fcd = [
    flat(a) for a in (vmax, msl, sst, t0, rh, rmax_b, lat, ckcd, cd)
]
vi = np.where(
    np.isfinite(fv)
    & (fv > 0.01)
...
def solve(i, buggy):
    ...
    try:
        _, _, _, rx = psmod.calculate_ps_ufunc(
            ...
            fl[i],
            fr[i],
            fck[i] if np.isfinite(fck[i]) else 0.9,
            fcd[i] if np.isfinite(fcd[i]) else 0.0015,
            0.002,
            1.2,
            "isothermal",
        )
...
feats = lambda i: np.column_stack(
    [np.ones_like(fr[i]), fr[i], fr[i] ** 2, fs[i], fv[i], fr[i] * fs[i]]
)
...
print(
    f"rho median {np.median(y):.4f}; fit held-out err median {np.median(e)*100:.3f}% 95pct {np.percentile(e,95)*100:.3f}%"
)
...
for ms, ml, pap in [(None, None, 17.06), (26.5, None, 7.94), (26.5, 30, 5.89)]:
    print(
        f"   sst>={ms}, |lat|<={ml}: {exc(norm_b,ms,ml):6.2f}% -> {exc(norm_f,ms,ml):6.2f}%  (paper {pap})"
    )
print(
...
print(
    f" max supersize ratio (AF): {np.nanmax(xb):.2f} -> {np.nanmax(xf):.2f}   (paper Ele 3.59)"
)
print("DONE")
