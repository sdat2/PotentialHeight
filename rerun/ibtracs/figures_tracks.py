...
for m, fn in fname.items():
    print("fresh-copy", fn, flush=True)
    shutil.copy2(f"{SRC}/{fn}", f"{DST}/{fn}")
...
vmap = {
    "r2": r(cps, "vmax"),
    "r3": r(xr.open_dataset(f'{DST}/{fname["r3"]}')[["vmax"]], "vmax"),
    "r1": r(xr.open_dataset(f'{DST}/{fname["r1"]}')[["vmax"]], "vmax"),
}
...
for meas, fn in fname.items():
    exact = np.load(os.path.join(EXACT_DIR, f"exact_{meas}.npz"))
    W = exact["W"].astype(bool)
    rf_cat1 = exact["rmax_fixed"]
    ...
    if os.path.exists(tf):
        ...
        t = time.time()
        tres = np.array(
            Parallel(n_jobs=3, backend="loky")(delayed(one)(vmax, i) for i in tidx)
        )
        np.savez(tf, idx=tidx, res=tres)
        print(
            f"[{meas}] re-solved {len(tidx)} track pts in {(time.time()-t)/60:.1f} min",
            flush=True,
        )
    nc = Dataset(f"{DST}/{fn}", "a")
    rm = nc.variables["rmax"]
    orig = np.asarray(rm[:], dtype="float64")
    sh = orig.shape
    flat = orig.ravel()
    ok = W & np.isfinite(rf_cat1)
    flat[ok] = rf_cat1[ok]
    tok = np.isfinite(tres)
    flat[tidx[tok]] = tres[tok]
    rm[:] = flat.reshape(sh)
    nc.close()
    print(f"[{meas}] patched cat1({int(ok.sum())})+track({int(tok.sum())})", flush=True)
# 4. symlink others
for fn in (
    ...
    l = f"{DST}/{fn}"
    if os.path.lexists(l) and not os.path.islink(l):
        pass
...
for nm, bs in STORMS:
    try:
        ib.plot_tc_example(
            name=nm.encode(), basin=bs.encode(), subbasin=b"WPAC", bbox=None
        )
        ok += 1
        print(f"  {nm} OK")
    except Exception as e:
        print(f"  {nm} FAIL: {type(e).__name__}: {str(e)[:100]}")
import glob
