...
The four knobs were swept independently over a filtered 190-case benchmark grid ($V_\text{max} \in \{20, 30, \ldots, 90\}$ m/s, $r_0 \in \{200, 400, \ldots, 2000\}$ km, $f \in \{3, 5, 7\} \times 10^{-5}$ s$^{-1}$), with rmax error measured relative to the pure-Python reference.  The grid was expanded from the original 75-case grid ($V_\text{max} \in \{30\text{–}70\}$ m/s, $r_0 \in \{400\text{–}1200\}$ km) to cover the full physically meaningful range including tropical-depression intensities and very large or very compact storms.

> **Excluded degenerate cases:** two categories of case are omitted from the benchmark grid to avoid inflating apparent error statistics with solver sensitivity that does not reflect normal usage:
>
> 1. **Small $r_0$ (inner Rossby number instability):**  The instability is triggered when the *inner* Rossby number $\mathrm{Ro}_\text{in} = V_\text{max}/(f\,r_\text{max})$ exceeds a critical value that depends only on $C_k/C_d$ (≈2419 for the default $C_k/C_d = 0.9$, independent of $w_\text{cool}$, $r_0$, or $f$).  This is a property of the ER11 non-dimensional solution: $r_\text{max} \cdot f/V_\text{max}$ is set by angular momentum balance and is a function of $C_k/C_d$ alone.  When $r_0$ is so small that the E04 outer curve must match the ER11 inner solution near its intrinsic $r_\text{max}$, the two curves become nearly tangent and the bisection termination side determines the answer (Python vs numba can differ by 15–45%).  At $r_0 \geq 300$ km every case in the grid is safely below this threshold (0.00% disagreement confirmed).  The worst case — $V_\text{max}=90$ m/s, $r_0=200$ km, $f=3\times10^{-5}$ s$^{-1}$ — is documented in the `TestMatlabRegression` section.
> 2. **Low-Ro (large $r_0$, weak storm, high $f$):** cases with outer Rossby number $\mathrm{Ro}_\text{out} = V_\text{max}/(f\,r_0) < 0.2$ are excluded — the storm is genuinely rotation-dominated, much larger than its Rossby length $V_\text{max}/f$.  This filters two cases: $(V_\text{max}, r_0, f) = (20\ \text{m/s},\ 1600\ \text{km},\ 7\times10^{-5}\ \text{s}^{-1},\ \mathrm{Ro}_\text{out}=0.179)$ and $(20\ \text{m/s},\ 2000\ \text{km},\ 7\times10^{-5}\ \text{s}^{-1},\ \mathrm{Ro}_\text{out}=0.143)$, both at equivalent latitude ~28.7°N.
