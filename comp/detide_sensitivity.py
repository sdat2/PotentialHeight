...
# --------------------------------------------------------------------------- #
def _utide_resid(
    wl: pd.Series, lat: float, method: str = "robust", trend: bool = True
) -> pd.Series:
    import utide
...
def _lowpass_resid(wl: pd.Series, kind: str = "godin") -> pd.Series:
    ...
    if kind == "doodson":
        ...
        return (
            pd.Series(wl.values - a, index=wl.index)
            .iloc[len(w) // 2 : -(len(w) // 2)]
            .dropna()
        )
...
METHODS: Dict[str, callable] = {
    "utide_robust_year": lambda wl, lat, st, win: _utide_resid(
        wl, lat, "robust", True
    ),  # baseline
...
def _latex_table(pooled: Dict[str, dict], path: str) -> None:
    ...
    for mid in TABLE_ORDER:
        ...
        if p:  # r to 3 dp: the harmonic deltas are sub-0.01
            lines.append(
                f"  {TABLE_LABELS[mid]} & ${p['r']:.3f}$ & ${p['bias']:+.2f}$ \\\\"
            )
...
def run(
    methods=None, limit: Optional[int] = None, write_table: bool = True
) -> pd.DataFrame:
    ...
    cols = list(
        dict.fromkeys(methods + ["skew_surge"])
    )  # methods then skew, no duplicates
    print(
        f"\nDe-tiding robustness on {len(out)} clean pairs "
        f"(simulated peak fixed; observed peak re-detided):\n"
    )
    print(
        f"{'method':22s} {'n':>4} {'r':>6} {'bias':>7} {'rmse':>6}  {'dr':>6} {'dbias':>7}"
    )
    ...
    for mname in cols:
        ...
        print(
            f"{mname:22s} {p['n']:>4} {p['r']:>6.3f} {p['bias']:>+7.3f} {p['rmse']:>6.3f}  "
...
def run_skew(limit: Optional[int] = None) -> pd.DataFrame:
    ...
    for _, row in clean.iterrows():
        ...
        try:
            ...
            continue
        rows.append(
            {"sim_peak": row["sim_peak"], "instantaneous": inst, "skew_surge": skew}
        )
    ...
    for col in ("instantaneous", "skew_surge"):
        ...
        print(
            f"{col:22s} {p['n']:>4} {p['r']:>6.3f} {p['bias']:>+7.3f} {p['rmse']:>6.3f}"
        )
    return out
...
def write_table_from_csv(path: Optional[str] = None) -> None:
    ...
    pooled = {
        row["method"]: {"r": float(row["r"]), "bias": float(row["bias"])}
        for _, row in res.iterrows()
    }
    _latex_table(
        pooled, path or os.path.join(C.PAPER_TEX_PATH, "comp_detide_table.tex")
    )
